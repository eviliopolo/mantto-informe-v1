...
from ..data.repositories.build_section3 import get_section3
...
class Section3Service:
    ...
    async def send_data_section(self, data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            anio = data.get("anio")
            mes = data.get("mes")
            section_id = data.get("section_id", "3")
            content = data.get("content")
            user_id = data.get("user_id", 1)
            name_file = data.get("name_file", f"INFORME_MENSUAL_{mes}_{anio}_V1.docx")
            
            level = data.get("level", 1)
            title = data.get("title", "3. INFORMES DE MEDICIÓN DE NIVELES DE SERVICIO (ANS)")
            
            collection = self._get_collection()
            datetime_now = datetime.now()
            
            document = await get_section3(anio, mes, user_id, name_file)
